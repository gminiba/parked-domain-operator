...
var _ = Describe("ParkedDomain Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			err := k8sClient.Get(ctx, typeNamespacedName, parkeddomain)
			if err != nil && errors.IsNotFound(err) {
				resource := &parkingv1alpha1.ParkedDomain{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ParkedDomain")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ParkedDomainReconciler{
				Client: k8sClient,
